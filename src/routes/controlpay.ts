...
function flattenQuery(q: unknown): Record<string, string> {
...
function buildControlpayWebhookPayload(request: {
  ...
  const merged: Record<string, unknown> = { ...flatQ, ...bodyObj };

  const refAlt = merged.intencaoVendaReferencia ?? merged.IntencaoVendaReferencia;
  if (
    refAlt != null &&
    String(refAlt).trim() !== "" &&
...
/**
 * POST /api/webhooks/controlpay/:coreCompanyId
 * Cadastre na PayGo a URL pública apontando para este path (UUID da Company no CORE).
 */
export async function controlpayRoutes(fastify: FastifyInstance) {
  fastify.post<{ Params: { coreCompanyId: string } }>(
    "/webhooks/controlpay/:coreCompanyId",
    async (request, reply) => {
      const t0 = Date.now();
      ...
      if (!company?.databaseName) {
        ...
        printControlPayWebhookBanner({
          ...
          message:
            "Empresa não encontrada no Core ou sem databaseName (tenant não provisionado).",
          ms: Date.now() - t0,
        });
      ...
      try {
        ...
        const { eventId, skippedDuplicate } =
          await createControlPayWebhookEventAndScheduleHydrate({
            tenant,
            coreCompanyId,
            payload,
          });
        ...
        if (e instanceof PrismaClientKnownRequestError && e.code === "P1000") {
          const hint =
            "Postgres recusou usuário/senha no tenant. Copie DATABASE_URL e CLIENT_DATABASE_URL do gestaowebprisma/.env para webhooks-fastify/.env, ou use Company.databasePass no Core.";
          printControlPayWebhookBanner({
            phase: "err",
            httpStatus: 500,
            code: "P1000",
