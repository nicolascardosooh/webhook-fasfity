...
export function printControlPayWebhookBanner(input) {
    if (input.phase === "start") {
        console.log("\n");
        lineHeavy();
        console.log(`${c.bold}${c.magenta}  ▶ ControlPay Webhook${c.reset}  ${c.dim}${new Date().toLocaleString("pt-BR", { hour12: false })}${c.reset}`);
        lineThin();
    ...
    if (input.phase === "ok") {
        const dup = input.skippedDuplicate ? ` ${c.yellow}(já existia — ignorado)${c.reset}` : "";
        console.log(`  ${c.green}${c.bold}✓ processado${c.reset}  eventId=${c.green}${input.eventId}${c.reset}${dup}  ${c.dim}${input.ms}ms${c.reset}`);
        lineHeavy();
        console.log("");
        return;
    }
...
//# sourceMappingURL=controlPayWebhookBanner.js.map