...
exports.efiRoutes = efiRoutes;
const db_1 = require("../lib/db");
const efiService_1 = require("../services/efiService");
const workerService_1 = require("../services/workerService");
async function efiRoutes(fastify) {
    ...
    fastify.post("/webhooks/efi", async (request, reply) => {
        ...
        try {
            ...
            const notificationDetails = await efiService_1.efiService.getNotification(notificationToken);
            request.log.info(`[EFI Webhook] Detalhes recuperados: ${JSON.stringify(notificationDetails)}`);
            ...
            request.log.info(`[EFI Webhook] Processando eventos para Assinatura ${subscription_id} (Ref: ${invoiceIdFromMetadata}). Pago: ${hasPaidEvent}, Ativo: ${hasActiveEvent}`);
            if (hasPaidEvent || hasActiveEvent) {
                request.log.info(`[EFI Webhook] Status elegível para processamento. Buscando fatura...`);
                const invoice = await db_1.prisma.invoice.findFirst({
                    where: {
                        OR: [
                ...
                request.log.info(`[EFI Webhook] Ação: Ativar Acesso? ${shouldActivateAccess} | Provisionar? ${shouldProvision}`);
                ...
                if (shouldProvision) {
                    try {
                        ...
                        const workerRes = await workerService_1.workerService.provisionDatabase(company.id);
                        if (workerRes.ok) {
